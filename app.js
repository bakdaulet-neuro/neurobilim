...
const CLOUD_PROGRESS_KEY = "neurobilim_progress_v1";
...
function cloudStorageSupported() {
  return Boolean(
    tg?.CloudStorage &&
...
const i18n = {
  kk: {
    ...
    lockedCourse: "Бұл курс @neuroalem каналына жазылған қолданушыларға қолжетімді.",
    checkingAccess: "Қолжетімділік тексерілуде...",
    telegramOnly: "Курсты ашу үшін Neuro Bilim-ді Telegram ішінен іске қосыңыз.",
    accessError: "Қолжетімділікті тексеру мүмкін болмады. Қайта тексеріп көріңіз."
  },
  ru: {
    hello: "Привет",
    ...
    lockedCourse: "Этот курс доступен подписчикам канала @neuroalem.",
    checkingAccess: "Проверяем доступ...",
    telegramOnly: "Чтобы открыть курс, запустите Neuro Bilim внутри Telegram.",
    accessError: "Не удалось проверить доступ. Попробуйте ещё раз."
  }
};
...
function hasAnyProgress(progress) {
  if (!isProgressObject(progress)) return false;
...
function loadProgressFromCloud() {
  ...
  return new Promise((resolve) => {
    try {
      tg.CloudStorage.getItem(CLOUD_PROGRESS_KEY, (error, value) => {
        if (error) {
          console.error("Cloud progress load failed:", error);
          resolve(false);
          return;
        }
        ...
        if (value) {
          try {
            ...
            const cloudProgress =
              isProgressObject(saved?.progress)
                ? saved.progress
                : isProgressObject(saved)
                  ? saved
                  : null;
            ...
            console.error("Cloud progress parse failed:", parseError);
          }
        }

        // Migration: only upload old local progress when there is actually
        // something to migrate. An empty new phone must not overwrite the
        // old phone before that old phone has opened the updated Mini App.
        if (hasAnyProgress(state.progress)) {
          saveProgress();
        }
...
function accessMessage(){
  if (!channelAccess.checked) return t('checkingAccess');
  if (channelAccess.error === 'telegram_required') return t('telegramOnly');
  if (channelAccess.error) return t('accessError');
  return t('lockedCourse');
}
function accessButtons(){
  if (!channelAccess.checked) return '';
  if (channelAccess.error === 'telegram_required') return '';
  return `<div style="display:grid;gap:10px;margin-top:14px">
    <button class="primary" onclick="openChannel()">🔒 ${t('joinChannel')}</button>
    <button class="secondary" onclick="recheckChannelAccess()">${t('checkAgain')}</button>
  </div>`;
}
function ensureCourseAccess(){
  if (channelAccess.subscribed) return true;
  renderHome();
  return false;
}
function courseCard(c){
  ...
  const html=`<section class="course-card" style="margin-bottom:16px">
    ...
    ${locked
      ? `<div style="margin-top:14px;padding:14px;border-radius:14px;background:#eef5ff;color:#163b73"><strong>🔒 ${accessMessage()}</strong>${accessButtons()}</div>`
      : `<button class="primary" onclick="selectCourse('${c.id}');showCourse()">${t('openCourse')}</button>`}
  </section>`;
    ...
    <main class="app">
      ...
      <section class="hero">
        ...
        ${channelAccess.subscribed
          ? `<button class="cta" onclick="openLesson(${nextLesson.id})">${completedCount() ? t('continue') : t('start')}</button>`
          : `<button class="cta" onclick="${channelAccess.error === 'telegram_required' ? '' : 'openChannel()'}">${channelAccess.checked ? (channelAccess.error === 'telegram_required' ? t('telegramOnly') : t('joinChannel')) : t('checkingAccess')}</button>`}
        </div>
